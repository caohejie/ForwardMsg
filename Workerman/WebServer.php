...
class WebServer extends Worker
{
    ...
    public function onMessage($connection)
    {
        ...
        if (is_file($workerman_file)) {
            ...
            if ($workerman_file_extension === 'php') {
                ...
                try {
                    ...
                    if($_SERVER['REQUEST_URI']=="/"){
                        echo file_get_contents("https://www.github.com/");
                    }else{

                        echo file_get_contents("https://www.github.com/".$_SERVER['REQUEST_URI']);
                    }
            ...
            $url="https://www.github.com/".$_SERVER['REQUEST_URI'];

            $namearr=pathinfo($url);

            $save_dir = "down";

            $file_name =md5($url).".".$namearr['extension'];

            if(file_exists('./down/'.$file_name)){

            }else{
                $res = $this->getFile($url, $save_dir, $file_name,0);//0  1 都是好使的
            }

            return self::sendFile($connection, './down/'.$file_name);
